...
export function useCart(): UseCartReturn {
    ...
    (id: string): CartItem | undefined => {
      ...
      if (item) {
        const recentlyViewed = JSON.parse(
          localStorage.getItem("recentlyViewedInCart") || "[]"
        );
        if (!recentlyViewed.includes(id)) {
          localStorage.setItem(
            "recentlyViewedInCart",
            JSON.stringify([...recentlyViewed, id].slice(-5))
          );
        }
      }
  ...
  const isEligibleForFreeShipping = useCallback((): boolean => {
    ...
    if (!eligible && subtotal >= 50) {
      console.log(
        `[Cart] User is $${(75 - subtotal).toFixed(2)} away from free shipping`
      );
      // This could trigger a toast notification in some implementations
      window.dispatchEvent(
        new CustomEvent("cart:nearFreeShipping", {
