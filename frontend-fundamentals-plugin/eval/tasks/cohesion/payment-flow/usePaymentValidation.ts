...
import { validateLuhn, isCardExpired, detectCardType } from "../utils/pricing";
import {
  PAYMENT_ERRORS,
  CARD_NUMBER_LENGTH,
  CVV_LENGTH,
...
export function usePaymentValidation() {
  ...
  const validateCvv = useCallback(
    (cvv: string, cardType?: string): boolean => {
  ...
  const validateCard = useCallback(
    async (data: CreditCardData): Promise<CardValidationResult> => {
      ...
      try {
        ...
        setError(
          "general",
          paymentError.message || PAYMENT_ERRORS.VALIDATION_FAILED
        );
